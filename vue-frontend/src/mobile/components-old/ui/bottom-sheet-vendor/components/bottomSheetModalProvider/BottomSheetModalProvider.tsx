...
const BottomSheetModalProviderWrapper = ({
    ...
    const getCurrentSheet = useCallback(() => {
        return sheetsQueueRef.current[sheetsQueueRef.current.length - 1]
    }, []);

    const getCurrentSheetKey = useCallback(() => {
        const s = sheetsQueueRef.current[sheetsQueueRef.current.length - 1]
        if (s)
            return s.key
    }, []);
    //#endregion
