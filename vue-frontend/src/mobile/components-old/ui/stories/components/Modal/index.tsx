...
import {ANIMATION_CONFIG, HEIGHT, LONG_PRESS_DURATION, WIDTH,} from '../../core/constants';
...
import Progress from "~ui/stories/components/Progress";
...
const StoryModal = forwardRef<StoryModalPublicMethods, StoryModalProps>((
    ...
    const onCloseDone = () => {
        bus.emit('bus:offers-insta.onClose')
        setVisible(false)
    }
    ...
    const backgroundAnimatedStyles = useAnimatedStyle(() => {
        ...
        const story = stories[userIndex.value]?.stories[storyIndex.value] || stories[userIndex.value]?.stories[0]
    ...
    const onGestureEvent = useAnimatedGestureHandler({
        ...
        onFinish: (e, ctx) => {
            ...
            if (ctx.vertical) {
                ...
                if (e.translationY > 100 || e.translationY < -200) {
    ...
    const isActive = useDerivedValue(() => true);
                        ...
                        >
                            ...
                            >
                                <Progress
                                    active={isActive}
                                    allStories={stories}
                                    activeOfferId={userId}
                                    activeSlideIndex={storyIndex}
                                    progress={animation}
                                    length={stories.length}
                                    progressColor={progressColor}
                                    progressActiveColor={progressActiveColor}
                                    containerWidth={wWidth - 55}
                                />

                                <View style={{width: 50, position: 'absolute', top: 0, right: 0}}>
                                    {
                                        CLOSER_BACK({
                                            color: '#000000',
                                            size: 32,
                                            onPress: () => {
                                                bus.emitter.emit('bus:offers-insta.hide')
                                            }
                                        })
                                    }
                                </View>
