...
import {ButtonSize, Incubator, Text, View} from "react-native-ui-lib";
...
const SliderWithInput: React.FC<{
    ...
    const onSliderValueChange = (num: number) => {
        setValueState(correctValue(num))
    }

    const onSliderSeekEnd = () => {
        onChangeValue(valueState)
    }
    ...
    return <View row centerV gap-10>
        <View centerH>
            <UiTextField
                value={valueState > 0 ? valueState.toString() : ''}
                presets={['outline', 'lg']}
                style={{
                    width: 50,
                    textAlign: 'center',
                    fontWeight: '900',
                    fontSize: !valueState ? 14 : 20
                }}
                ...
                !vorder.bonusesAvailable ?
                    ...
                    <View gap-20 marginB-10>
                        ...
                        <View paddingH-10 row centerV>
                            <Text center text-sm-r-lh0>До 30% от суммы заказа</Text>
                            <View marginL-auto>
                                <UiBtn
