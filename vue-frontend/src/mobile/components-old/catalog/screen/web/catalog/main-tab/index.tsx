...
const MainTabComponent: React.FC<TProps> = (props) => {
    ...
    const renderUserBoxOrder = useCallback((order: OrderModel) => {
        return <View gap-6>
            ...
            <View
                ...
                br-sm
                style={{
                    backgroundColor: order.CSTATUS_COLOR,
                }}
            >
                <Text
                    text-sm-m-lh0
    ...
    const coms = {
        ...
        offersCommon: (p: PartialWithKey<OffersCommonSliderProps> = {}) =>
            ...
            <OffersCommonSlider key={'offersCommon'} items={offer.commonOffersSorted} {...p}/>,

        popular: (p: PartialWithKey<CatalogProductsSliderProps> = {}) =>
            !!catalog.popularProductsShort.length &&
            <CatalogProductsSlider key={'popular'} products={catalog.popularProductsShort}
                                   excludeFlags={['hit']} {...p}/>,

        fav: (p: PartialWithKey<CatalogProductsSliderProps> = {}) =>
            fav.haveItems &&
            <CatalogProductsSlider key={'fav'} products={fav.products} excludeFlags={['fav']} {...p}/>,

        menu: (p: PartialWithKey<TCatalogMenuProps> = {}) =>
            <CatalogMenu key={'menu'} items={catalog.menuItems} marginH-7 {...p}/>,

        special: (p: PartialWithKey<CatalogProductsSliderProps> = {}) =>
            !!catalog.requiredPriceProducts.length &&
            <CatalogProductsSlider key={'special'} products={catalog.requiredPriceProducts} {...p}/>
    }
        ...
        <View gap-12 marginT-15 paddingH-screenH>
            <View row gap-16 centerV>
                ...
                <View flexS>
                    <Text
                        ...
                        ellipsizeMode={'clip'}
                        white
                    >Привет111, {user.user?.GREETING_NAME || 'Гость'}!</Text>
                </View>
            </View>
            <View
                style={{
            ...
            >
                <UiTextField
                    ...
                    onPress={() => {
                        user.setAuthed()
                        //notice.showNewNotices(true)
                        //noticesDialog.show()
                        //router.push('/catalog/search?focus=1')
                    }}
