...
export const useSelect = <T>({
    ...
    const onPressSelectControl: OnPressSelectControlType = async () => {
        ...
        if (optionsResolver && (!state.optionsResolved || !optionsData.length)) {
            return
        }
    ...
    useImperativeHandle(
        ...
        (): SelectRef<T> => ({
            ...
            search: (query) => {
                state.searchValue
                //searchInputRef.current?.setNativeProps({ text: query })
            },
            selectOption: (option: any) => {

                dispatch({
                    type: 'selectOption',
                    payload: { selectedOption: option, selectedOptionIndex: 0 },
                });
                dispatch({
                    type: 'setSearchValue',
