...
const useScrollTo = Hooks.useScrollTo
import {ScrollToItemProps} from "./useScrollToItem.types";

export let OffsetType = /*#__PURE__*/function (OffsetType: any) {
...
const useScrollToItem = (props: ScrollToItemProps<any>) => {
  const {
  ...
  const itemsWidths = useRef<any>(_times(itemsCount, () => null));
  ...
  const onItemLayout = useCallback((event: any, index: number) => {
    const {
      width
