...
type CountdownControllers = {
  ...
  /** Reset the countdown. */
  resetCountdown: () => void
  setCountStart: (seconds: number) => void
  startCountdownWithTime: (seconds: number) => void
}
...
export function useCountdown({
  ...
  const startCountdownWithTime = useCallback((seconds: number) => {
    setCountStart(seconds)
    startCountdown()
  }, [startCountdown])


  return [count, { setCountStart, startCountdownWithTime, startCountdown, stopCountdown, resetCountdown }]
}
