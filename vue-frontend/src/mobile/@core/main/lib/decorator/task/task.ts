 ...
 * Task status.
 */
export type TaskStatus = 'initial' | 'pending' | 'resolved' | 'rejected'
...
export interface TaskState<A extends any[], R> {
    ...
    /**
     * Convenience getter for `state === 'initial'`.
     */
    ...
    /**
     * Convenience getter for `state === 'resolved_once'`.
     */
    resolvedOnce: boolean
...
export interface TaskMethods<A extends any[], R> {
    ...
    polling(interval?: number | boolean): void
}
...
export interface TaskCreator<K extends keyof TaskOptions<any, any>>
    extends MethodDecorator, PropertyDecorator {
    /**
...
function createTask<A extends any[], R>(
    ...
    const taskStateSchema: TaskState<A, R> = {
        ...
        resolvedOnce: false,
        get initial() {
            return taskState.state === 'initial'
        },
        get pending() {
...
function setupTask<A extends any[], R>(
    ...
    const setup = (func: any) => setupTask(func, taskState, taskStateSchemaKeys, opts)
    ...
    Object.assign(fn, {
        ...
        polling: action('polling', (interval?: number | false) => {

            if (taskState.pollingTimer)
                clearInterval(taskState.pollingTimer)

            if (interval !== false) {
                interval = typeof interval === 'number' ? interval : (opts.pollingInterval || 60 * 1000)
                console.log('Start polling task "' + opts.name + '" with interval', interval)
                taskState.pollingTimer = setInterval(() => {
                    console.log('Polling task "' + opts.name + '" ' + dayjs().format('HH:mm:ss'))
                    // @ts-ignore
                    fn()
                }, interval)
            }
